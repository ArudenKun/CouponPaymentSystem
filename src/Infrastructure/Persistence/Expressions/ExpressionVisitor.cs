...
namespace CouponPaymentSystem.Infrastructure.Persistence.Expressions;
...
#pragma warning restore CS8603 // Possible null reference return.
    ...
    protected virtual Expression VisitBinary(BinaryExpression binaryExp)
    {
        ...
        if (
            left != binaryExp.Left
            || right != binaryExp.Right
            || conversion != binaryExp.Conversion
        )
        {
    ...
    protected virtual Expression VisitConditional(ConditionalExpression conditionalExp)
    {
        ...
        if (
            test != conditionalExp.Test
            || ifTrue != conditionalExp.IfTrue
            || ifFalse != conditionalExp.IfFalse
        )
        {
    ...
    {
        IEnumerable<ElementInit> initializers = VisitElementInitializerList(
            listBindingExp.Initializers
        );
