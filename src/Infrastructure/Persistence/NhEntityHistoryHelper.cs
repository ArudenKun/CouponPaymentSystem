...
internal class NhEntityHistoryHelper
    : EntityHistoryHelperBase,
        ...
        ITransientDependency
{
    ...
    {
        _entityHistoryStore = new Lazy<IEntityHistoryStore>(
            () =>
        ...
        _clientInfoProvider = new Lazy<IClientInfoProvider>(
            () =>
        ...
        _entityChangeSetReasonProvider = new Lazy<IEntityChangeSetReasonProvider>(
            () =>
    ...
    public virtual void SaveChangeSet(Guid sessionId)
    {
        ...
        if (!EntityChangeSets.TryGetValue(sessionId, out var changeSet))
            return;
    ...
    {
        var entityChangeSet = EntityChangeSets.GetOrAdd(
            sessionId,
            _ => new EntityChangeSet
            {
                Reason = EntityChangeSetReasonProvider.Reason.TruncateWithPostfix(
                    EntityChangeSet.MaxReasonLength
                ), // todo: add HTTP METHOD to the beginning of the URI string

                // Fill "who did this change"
                BrowserInfo = ClientInfoProvider.BrowserInfo.TruncateWithPostfix(
                    EntityChangeSet.MaxBrowserInfoLength
                ),
                ClientIpAddress = ClientInfoProvider.ClientIpAddress.TruncateWithPostfix(
                    EntityChangeSet.MaxClientIpAddressLength
                ),
                ClientName = ClientInfoProvider.ComputerName.TruncateWithPostfix(
                    EntityChangeSet.MaxClientNameLength
                ),
    ...
    private ICollection<EntityPropertyChange> GetPropertyChanges(
    ...
    )
    {
        ...
        var dirtyPropertyIndexes = @event.Persister.FindDirty(
            @event.State,
            new object[@event.State.Length],
            @event.Entity,
            @event.Session
        );
        ...
        foreach (var dirtyPropertyIndex in dirtyPropertyIndexes)
        {
            ...
            var propertyInfo = @event
                .Entity.GetType()
                .GetProperty(@event.Persister.PropertyNames[propertyIndex]);

            var newValue = @event.State[propertyIndex];
            var propertyName = property.Name;

            if (
                @event.State[propertyIndex] != null
                && Convert.GetTypeCode(@event.State[propertyIndex]) == TypeCode.Object
            )
            {
                var dirtyFieldProperty = @event
                    .Persister.EntityMetamodel.Type.GetProperties()
            ...
            {
                propertyChanges.Add(
                    CreateEntityPropertyChange(null, newValue, propertyName, propertyInfo)
                );
            }
        }
    ...
    )
    {
        ...
        foreach (var dirtyPropertyIndex in dirtyPropertyIndexes)
        {
            ...
            if (
                @event.OldState[propertyIndex] != null
            ...
            {
                propertyChanges.Add(
                    CreateEntityPropertyChange(oldValue, newValue, propertyName, propertyInfo)
                );
            }
        }
    ...
    )
    {
        ...
        var dirtyPropertyIndexes = @event.Persister.FindDirty(
            new object[@event.DeletedState.Length],
            @event.DeletedState,
            @event.Entity,
            @event.Session
        );
        ...
        foreach (var dirtyPropertyIndex in dirtyPropertyIndexes)
        {
            ...
            if (
                @event.DeletedState[propertyIndex] != null
            ...
            {
                propertyChanges.Add(
                    CreateEntityPropertyChange(oldValue, null, propertyName, propertyInfo)
                );
            }
        }
    ...
    private EntityPropertyChange CreateEntityPropertyChange(
        object? oldValue,
        object? newValue,
        string propertyName,
    ...
    )
    {
        ...
        {
            PropertyName = propertyName.TruncateWithPostfix(
                EntityPropertyChange.MaxPropertyNameLength
            ),
