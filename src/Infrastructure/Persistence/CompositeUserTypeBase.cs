...
using CouponPaymentSystem.Infrastructure.Persistence.Expressions;
using NHibernate;
using NHibernate.Engine;
using NHibernate.Type;
using NHibernate.UserTypes;

namespace CouponPaymentSystem.Infrastructure.Persistence;
...
public abstract class CompositeUserTypeBase<T> : ICompositeUserType
{
    ...
    protected virtual void MapProperty(Expression<Func<T, object>> property)
    {
        Guard.Against.Null(
            property,
            "Expected a non-null Expression that represents the property to map."
        );
    ...
    )
    {
        ...
        for (var i = 0; i < names.Length; i++)
            values[i] = NHibernateUtil
                .GuessType(_properties[i].PropertyType)
    ...
    )
    {
        ...
        for (var i = 0; i < _properties.Count; i++)
        {
            ...
            NHibernateUtil
                .GuessType(property.PropertyType)
                .NullSafeSet(cmd, propValue, propIndex, session);
            propIndex++;
        }
    }
