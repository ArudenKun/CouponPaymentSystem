...
/// </summary>
internal class NotificationStore : INotificationStore, ITransientDependency
{
    ...
    )
    {
        ...
        if (!targetNotifiers.IsNullOrEmpty())
        {
            ...
            var targetNotifierList = targetNotifiers.Split(
                NotificationInfo.NotificationTargetSeparator
            );
            foreach (var targetNotifier in targetNotifierList)
            {
                targetNotifierPredicate = targetNotifierPredicate.Or(e =>
                    e.TargetNotifiers.Contains(targetNotifier)
                );
            }
    ...
    )
    {
        await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                var userNotification = await _userNotificationRepository.FirstOrDefaultAsync(
                    userNotificationId
                );
    ...
    )
    {
        _unitOfWorkManager.WithUnitOfWork(() =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                var userNotification = _userNotificationRepository.FirstOrDefault(
                    userNotificationId
                );
                if (userNotification == null)
                {
    ...
    )
    {
        await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(user.TenantId))
            {
                var userNotifications = await _userNotificationRepository.GetAllListAsync(un =>
    ...
    public virtual void UpdateAllUserNotificationStates(
        UserIdentifier user,
        UserNotificationState state
    )
    {
        _unitOfWorkManager.WithUnitOfWork(() =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(user.TenantId))
            {
                var userNotifications = _userNotificationRepository.GetAllList(un =>
                    un.UserId == user.UserId
                );
    ...
    )
    {
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            {
                var query =
                    from userNotificationInfo in await _userNotificationRepository.GetAllAsync()
                    join tenantNotificationInfo in await _tenantNotificationRepository.GetAllAsync()
    ...
    )
    {
        return _unitOfWorkManager.WithUnitOfWork(() =>
        {
            ...
            {
                var query =
                    from userNotificationInfo in _userNotificationRepository.GetAll()
    ...
    public virtual async Task<UserNotificationInfoWithNotificationInfo?> GetUserNotificationWithNotificationOrNullAsync(
        int? tenantId,
        Guid userNotificationId
    )
    {
    ...
    public async Task<
        List<GetNotificationsCreatedByUserOutput>
    > GetNotificationsPublishedByUserAsync(
        UserIdentifier user,
        string notificationName,
        DateTime? startDate,
        DateTime? endDate
    )
    {
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(user.TenantId))
            {
                var queryForNotPublishedNotifications = (
                    await _notificationRepository.GetAllAsync()
                ).Where(n =>
                    n.CreatorUserId == user.UserId && n.NotificationName == notificationName
                );

                if (startDate.HasValue)
                {
                    queryForNotPublishedNotifications = queryForNotPublishedNotifications.Where(x =>
                ...
                var queryForPublishedNotifications = _tenantNotificationRepository
                    .GetAll()
                    .Where(n =>
                        n.CreatorUserId == user.UserId && n.NotificationName == notificationName
                    );
                ...
                if (startDate.HasValue)
                {
                    queryForPublishedNotifications = queryForPublishedNotifications.Where(x =>
                        x.CreationTime >= startDate
                    );
                }
                ...
                queryForPublishedNotifications = queryForPublishedNotifications.OrderByDescending(
                    n => n.CreationTime
                );
