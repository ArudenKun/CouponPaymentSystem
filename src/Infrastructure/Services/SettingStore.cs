...
using CouponPaymentSystem.Infrastructure.Extensions;
...
/// </summary>
internal class SettingStore : ISettingStore, ITransientDependency
{
        ...
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    ...
                    return settingList.Select(s => s.ToSettingInfo()).ToList();
                }
            }
        });
    }
            ...
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    return _settingRepository
                        .GetAllList(s => s.UserId == userId && s.TenantId == tenantId)
    ...
    public virtual async Task<SettingInfo?> GetSettingOrNullAsync(
        int? tenantId,
        long? userId,
        string name
    )
    ...
    public virtual async Task DeleteAsync(SettingInfo settingInfo)
    {
        await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(settingInfo.TenantId))
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    await _settingRepository.DeleteAsync(s =>
    ...
    public virtual void Update(SettingInfo settingInfo)
    {
        _unitOfWorkManager.WithUnitOfWork(() =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(settingInfo.TenantId))
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    var setting = _settingRepository.FirstOrDefault(s =>
                        s.TenantId == settingInfo.TenantId
                        && s.UserId == settingInfo.UserId
                        && s.Name == settingInfo.Name
                    );
