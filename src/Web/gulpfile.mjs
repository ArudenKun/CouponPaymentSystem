...
var viewScripts = globbySync([
    "./Content/view-resources/**/*.js",
    "!./Content/view-resources/**/*.min.js",
]);

var viewStyles = globbySync([
    "./Content/view-resources/**/*.css",
    "./Content/view-resources/**/*.less",
    "!./Content/view-resources/**/*.min.css",
]);
...
function fillScriptBundles() {
    ...
    for (var i = 0; i < viewScripts.length; i++) {
        var viewScriptName = viewScripts[i].replace("./Content/", "");
...
function fillStyleBundles() {
    ...
    for (var j = 0; j < viewStyles.length; j++) {
        var viewStyleName = viewStyles[j].replace("./Content/", "");

        if (viewStyleName.indexOf(".css") >= 0) {
            styleEntries[viewStyleName.replace(".css", ".min.css")] = [
                path.resolve(__dirname, "Content/" + viewStyleName),
            ];
        }
...
function createScriptBundle(script) {
    ...
    return stream
        ...
        .pipe(gulp.dest("Content/" + bundlePath));
}
...
function createStyleBundle(style) {
    ...
    var options = {
        url: function (asset) {
            ...
            var outputPath = path.join(__dirname, "/Content/" + outputFolder + "/");
            gulp.src(asset.absolutePath).pipe(gulp.dest(outputPath));
            return "/" + "Content" + "/" + outputFolder + "/" + fileName;
        },
    };
