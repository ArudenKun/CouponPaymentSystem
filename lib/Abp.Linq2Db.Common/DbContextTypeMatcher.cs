...
[PublicAPI]
public abstract class DbContextTypeMatcher<TBaseDbContext>
        ...
        ISingletonDependency
{
    ...
    public virtual Type GetConcreteType(Type sourceDbContextType)
    {
        ...
        var multiTenancySideContexes = GetMultiTenancySideContextTypes(
            allTargetTypes,
            currentTenancySide
        );
        ...
        if (multiTenancySideContexes.Count > 1)
        {
            return GetDefaultDbContextType(
                multiTenancySideContexes,
                sourceDbContextType,
                currentTenancySide
            );
        }
    ...
    private static List<Type> GetMultiTenancySideContextTypes(
        List<Type> dbContextTypes,
        MultiTenancySides tenancySide
    )
    {
    ...
    private static Type GetDefaultDbContextType(
        List<Type> dbContextTypes,
        Type sourceDbContextType,
        MultiTenancySides tenancySide
    )
    {
        var filteredTypes = dbContextTypes
            .Where(type =>
                !type.GetTypeInfo().IsDefined(typeof(AutoRepositoryTypesAttribute), true)
            )
            .ToList();
