...
namespace Abp.Linq2Db.Common.Repositories;

[AutoInterface]
[PublicAPI]
public class Linq2DbGenericRepositoryRegistrar
    : ILinq2DbGenericRepositoryRegistrar,
        ITransientDependency
{
    ...
    public Linq2DbGenericRepositoryRegistrar(IDbContextEntityFinder dbContextEntityFinder)
    {
    ...
    )
    {
        ...
        if (primaryKeyType == typeof(int))
        {
            ...
            {
                var implType =
                    repositoryImplementation.GetGenericArguments().Length == 1
        ...
        {
            var implType =
                repositoryImplementationWithPrimaryKey.GetGenericArguments().Length == 2
                    ? repositoryImplementationWithPrimaryKey.MakeGenericType(
                        entityType,
                        primaryKeyType
                    )
                    : repositoryImplementationWithPrimaryKey.MakeGenericType(
    ...
    )
    {
        foreach (var entityTypeInfo in _dbContextEntityFinder.GetEntityTypeInfos(dbContextType))
        {
            ...
            if (primaryKeyType == typeof(int))
            {
                var genericRepositoryType = repositoryInterface.MakeGenericType(
                ...
                if (!iocManager.IsRegistered(genericRepositoryType))
                {
                    var implType =
                        repositoryImplementation.GetGenericArguments().Length == 1
                            ...
                            : repositoryImplementation.MakeGenericType(
            ...
            var genericRepositoryTypeWithPrimaryKey =
                repositoryInterfaceWithPrimaryKey.MakeGenericType(
                    entityTypeInfo.EntityType,
                    primaryKeyType
                );
            if (!iocManager.IsRegistered(genericRepositoryTypeWithPrimaryKey))
