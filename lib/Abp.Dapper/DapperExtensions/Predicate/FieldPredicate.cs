...
public class FieldPredicate<T> : ComparePredicate, IFieldPredicate
{
    ...
    )
    {
        ...
        {
            result = sqlGenerator.Configuration.Dialect.GetDatabaseFunctionString(
                DatabaseFunction,
                result,
    ...
    private string GetColumnName(
        ISqlGenerator sqlGenerator,
        ref Type parentType,
        ref string parameterPropertyName,
        bool isDml
    )
    {
        string result;
    ...
    {
        var p = DapperExtensionsReflectionHelper.GetParameter(
            typeof(T),
            sqlGenerator,
            PropertyName,
            value
    ...
    {
        parameterPropertyName = string.IsNullOrEmpty(parameterPropertyName)
            ? PropertyName
            : parameterPropertyName;

        var propParam = DapperExtensionsReflectionHelper.GetParameter(
            parentType ?? typeof(T),
            sqlGenerator,
            parameterPropertyName,
            Value
        );
        return parameters.SetParameterName(
            propParam,
    ...
    )
    {
        ...
        return @params.Aggregate(
            new StringBuilder(),
            (sb, s) => sb.Append(sb.Length != 0 ? ", " : string.Empty).Append(s),
            sb => sb.ToString()
        );
    }
    ...
    )
    {
        ...
        if (Value is IEnumerable values && !(Value is string))
        {
            ...
            return string.Format(
                ...
                GetParameterString(sqlGenerator, parameters, values)
            );
        }
        ...
        {
            format =
                (sqlGenerator.Configuration.Dialect is OracleDialect)
                    ? "BITAND({0}, {2}) {1} {2}"
                    : "{0}&{2} {1} {2}";
        }
        else
        ...
        return string.Format(
            ...
            columnName,
            GetOperatorString(),
            GetParameterName(sqlGenerator, parameters, parameterPropertyName, parentType)
        );
    }
