...
public class PropertyPredicate<T, T2> : ComparePredicate, IPropertyPredicate
{
    ...
    )
    {
        ...
        {
            columnName = sqlGenerator.Configuration.Dialect.GetDatabaseFunctionString(
                LeftDatabaseFunction,
                columnName,
                LeftDatabaseFunctionParameters
            );
        }

        if (!RigthDatabaseFunction.Equals(DatabaseFunction.None))
        {
            columnName2 = sqlGenerator.Configuration.Dialect.GetDatabaseFunctionString(
                RigthDatabaseFunction,
                columnName2,
                RigthDatabaseFunctionParameters
            );
        }
