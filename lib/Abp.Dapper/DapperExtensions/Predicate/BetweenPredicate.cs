...
public class BetweenPredicate<T> : BasePredicate, IBetweenPredicate
{
    ...
    )
    {
        ...
        var parameter1 = DapperExtensionsReflectionHelper.GetParameter(
            typeof(T),
            sqlGenerator,
            PropertyName,
            Value.Value1
        );
        var parameter2 = DapperExtensionsReflectionHelper.GetParameter(
            typeof(T),
            sqlGenerator,
            PropertyName,
            Value.Value2
        );
        var propertyName1 = parameters.SetParameterName(
            parameter1,
            sqlGenerator.Configuration.Dialect.ParameterPrefix
        );
        var propertyName2 = parameters.SetParameterName(
            parameter2,
            sqlGenerator.Configuration.Dialect.ParameterPrefix
        );
