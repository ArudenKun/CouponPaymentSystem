...
public class ReferenceMap<T> : IReferenceMap<T>
{
    ...
    {
        var binaries = DapperExtensionsReflectionHelper.GetBinaryExpressionsFromUnary(
        ...
        foreach (BinaryExpression binary in binaries)
        {
            ...
            var leftKey = new PropertyKey(
                (PropertyInfo)leftMember,
                leftMember.DeclaringType,
                leftMember.Name
            );
            var rightKey = new PropertyKey(
                (PropertyInfo)rightMember,
                rightMember.DeclaringType,
                rightMember.Name
            );
            var comparator = DapperExtensionsReflectionHelper.GetRelacionalComparator(
                binary.NodeType,
                Name
            );

            var referenceProperty = new ReferenceProperty<TEntity>(
                PropertyInfo,
                ParentIdentity,
                Identity
            );
