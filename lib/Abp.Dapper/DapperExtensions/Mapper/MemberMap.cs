...
using Abp.Dapper.DapperExtensions.Extensions;

namespace Abp.Dapper.DapperExtensions.Mapper;
...
public class MemberMap : IMemberMap
{
    ...
    public MemberMap(
        ...
        IMemberMap parent = null
    )
    {
        MemberInfo = memberInfo;
        ColumnName = isReference
            ? classMapper
                .Properties.Where(x => x.MemberInfo == MemberInfo)
    ...
    public MemberMap Key(KeyType keyType)
    {
        if (Ignored)
        {
            throw new ArgumentException(
                string.Format("'{0}' is ignored and cannot be made a key field. ", Name)
            );
        }
        ...
        {
            throw new ArgumentException(
                string.Format("'{0}' is readonly and cannot be made a key field. ", Name)
            );
        }
    ...
    public MemberMap Ignore()
    {
        ...
        {
            throw new ArgumentException(
                string.Format("'{0}' is a key field and cannot be ignored.", Name)
            );
        }
    ...
    public MemberMap ReadOnly()
    {
        ...
        {
            throw new ArgumentException(
                string.Format("'{0}' is a key field and cannot be marked readonly.", Name)
            );
        }
    ...
    public MemberMap Size(int size)
    {
        ...
        {
            throw new ArgumentException(
                string.Format("'{0}' cannot have a negative field length.", Name)
            );
        }
