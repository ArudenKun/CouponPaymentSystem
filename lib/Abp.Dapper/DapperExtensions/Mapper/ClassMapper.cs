...
public class ClassMapper<T> : IClassMapper<T>
{
    ...
    protected virtual void AutoMap(Func<Type, PropertyInfo, bool> canMap)
    {
        ...
        )
        {
            if (
                Properties.Any(p =>
                    p.Name.Equals(propertyInfo.Name, StringComparison.InvariantCultureIgnoreCase)
                )
            )
            {
                continue;
        ...
        keyMap?.Key(
            PropertyTypeKeyTypeMapping.ContainsKey(keyMap.MemberType)
        ...
        if (GuardForDuplicatePropertyMap(result))
        {
            result = (MemberMap)
                Properties.FirstOrDefault(p =>
                    p.Name.Equals(result.Name) && p.ParentProperty == result.ParentProperty
                );
        }
        else
        {
    ...
    private void GuardForDuplicateReferenceMap(ReferenceMap<T> result)
    {
        if (References.Any(p => p.Name.Equals(result.Name)))
        {
            throw new ArgumentException(
                $"Duplicate mapping for reference property {result.Name} detected."
            );
        }
    }
