...
public static class Predicates
{
    ...
    public static IFieldPredicate Field<T>(
        Expression<Func<T, object>> expression,
        Operator op,
        object value,
        bool not = false,
        bool useColumPrefix = true,
        DatabaseFunction databaseFunction = DatabaseFunction.None,
        string databaseFunctionParameters = ""
    )
    {
    ...
    {
        var properties = DapperExtensionsReflectionHelper.GetNestedProperties<T>(
            propertyName,
            '.',
            out string propertyInfoName
        );
    ...
    public static IPropertyPredicate Property<T, T2>(
        Expression<Func<T, object>> expression,
        Operator op,
        Expression<Func<T2, object>> expression2,
        bool not = false,
        bool useLeftColumPrefix = true,
        bool useRightColumPrefix = true,
        DatabaseFunction leftDatabaseFunction = DatabaseFunction.None,
        string leftDatabaseFunctionParameters = "",
        DatabaseFunction rigthDatabaseFunction = DatabaseFunction.None,
        string rigthDatabaseFunctionParameters = ""
    )
    {
        var propertyInfo = DapperExtensionsReflectionHelper.GetProperty(expression) as PropertyInfo;
        var propertyInfo2 =
            DapperExtensionsReflectionHelper.GetProperty(expression2) as PropertyInfo;
    ...
    /// <param name="not">Effectively inverts the comparison operator. Example: WHERE FirstName &lt;&gt; LastName </param>
    public static IBetweenPredicate Between<T>(
        Expression<Func<T, object>> expression,
    ...
    public static ISort Sort<T>(Expression<Func<T, object>> expression, bool ascending = true)
    {
        ...
        return new Sort
        {
            PropertyName = propertyInfo.Name,
            Ascending = ascending,
            Properties = DapperExtensionsReflectionHelper.GetNestedProperties<T>(
                propertyInfo.Name,
                '.',
                out var _
            ),
    ...
    {
        var propertyInfos = DapperExtensionsReflectionHelper.GetNestedProperties<T>(
            propertyName,
            '.',
            out var propertyInfoName
        );
    ...
    public static IInPredicate In<T>(
        Expression<Func<T, object>> expression,
    ...
    /// </example>
    /// <returns></returns>
    public static IVirtualPredicate VirtualPredicate(
        string comparable,
        Operator op,
        object value,
        bool not = false
