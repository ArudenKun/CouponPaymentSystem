...
using System.Text;
using Abp.Dapper.DapperExtensions.Enums;
...
public static class DapperExtensionsReflectionHelper
{
    ...
    public static IList<PropertyInfo> GetNestedProperties<T>(
    ...
    )
    {
        ...
        foreach (var propName in properties)
        {
            ...
            _propertyInfoName +=
                propertyInfo.Name + ((index < properties.Length) ? delimiter.ToString() : "");
    ...
    public static BinaryExpression GetBinaryExpressions(
        BinaryExpression expression,
        out IList<BinaryExpression> binaries
    )
    {
    ...
    public static Comparator GetRelacionalComparator(ExpressionType type, string name = "")
    {
        return type switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(
                string.Format("Comparator option is not valid for property {0} detected.", name)
            ),
        };
    }
    ...
    {
        string _typeName = string.Format(
            "{0}{1}",
            typeName,
            DapperExtensions.GetNextGuid().ToString().Substring(0, 8)
        );
    ...
    )
    {
        ...
        if (map == null)
            throw new NullReferenceException(
                String.Format("Map was not found for {0}", entityType)
            );
        ...
        if (propertyMap == null)
            throw new NullReferenceException(
                String.Format("{0} was not found for {1}", entityPropertyName, entityType)
            );
    ...
    public static PropertyInfo GetPropertyInfo(Type type, string propertyName)
    {
        ...
        )
        {
            throw new System.Exception(
                $"Property name '{propertyName}' not exists inside {type.FullName}. \n"
