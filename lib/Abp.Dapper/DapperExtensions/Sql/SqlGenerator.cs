...
using System.Text;
using Abp.Dapper.DapperExtensions.Mapper;
...
public interface ISqlGenerator
{
    ...
    string Select(
        ...
        IList<IProjection> colsToSelect,
...
public class SqlGeneratorImpl : ISqlGenerator
{
    ...
    private string GetPartitionBy()
    {
        ...
        {
            var keyTable = AllColumns
                .Where(c => c.SimpleAlias.Equals(partitionBy))
                .Select(c => c.Table)
                .First();
            ...
            var orderBy = sort.Select(s =>
                {
                    ...
                    return GetColumnName(map, propertyName, false)
    ...
    public virtual string SelectPaged(
        IClassMapper classMap,
        IPredicate predicate,
        IList<ISort> sort,
        int page,
    ...
    )
    {
        if (sort?.Any() != true)
        {
            throw new ArgumentNullException(
                nameof(Sort),
                $"{nameof(Sort)} cannot be null or empty."
            );
        }
        ...
        var innerSql = new StringBuilder(
            Select(classMap, predicate, sort, parameters, colsToSelect, includedProperties)
        );

        var partitionBy = GetPartitionBy();

        return Configuration.Dialect.GetPagingSql(
            innerSql.ToString(),
            page,
            resultsPerPage,
            parameters,
    ...
    public virtual string SelectSet(
        IClassMapper classMap,
        IPredicate predicate,
        IList<ISort> sort,
        int firstResult,
    ...
    )
    {
        ...
        var innerSql = new StringBuilder(
            $"SELECT {BuildSelectColumns(classMap, colsToSelect, includedProperties)} FROM {GetTables(classMap, parameters, includedProperties)}"
        );
        ...
        var orderBy = sort.Select(s =>
                GetColumnName(classMap, s.PropertyName, false) + (s.Ascending ? " ASC" : " DESC")
            )
            .AppendStrings();
        innerSql.Append(" ORDER BY ").Append(orderBy);

        return Configuration.Dialect.GetSetSql(
            innerSql.ToString(),
            firstResult,
            maxResults,
            parameters
    ...
    )
    {
        ...
        if (includedProperties?.Count > 0 && Configuration.Dialect.SupportsCountOfSubquery)
        {
            ...
            sql.Append("SELECT ")
                .Append(resultSet)
                .Append(" FROM ")
                .Append(GetTables(classMap, parameters, includedProperties));
    ...
    public virtual string Insert(IClassMapper classMap)
    {
        ...
        var parameters = AllColumns
            .Where(p =>
                !(
                    ...
                    || p.Property.KeyType == KeyType.Identity
                    || p.Property.KeyType == KeyType.TriggerIdentity
                )
            )
        ...
        var triggerIdentityColumn = classMap
            .Properties.Where(p => p.KeyType == KeyType.TriggerIdentity)
            .ToList();
        var sequenceIdentityColumn = classMap
            .Properties.Where(p => p.KeyType == KeyType.SequenceIdentity)
            .ToList();
        ...
        if (triggerIdentityColumn.Count > 0)
        {
            ...
            sql +=
                $" RETURNING {triggerIdentityColumn.Select(p => GetColumnName(classMap, p, false, includePrefix: false)).First()} INTO {Configuration.Dialect.ParameterPrefix}IdOutParam";
        }
    ...
    )
    {
        if (predicate == null)
        {
            throw new ArgumentNullException(
                nameof(predicate),
                $"{nameof(predicate)} cannot be null."
            );
        }
        ...
        var columns = (
            ignoreAllKeyProperties
                ? AllColumns.Where(p =>
                    !(p.Property.Ignored || p.Property.IsReadOnly)
                    && p.Property.KeyType == KeyType.NotAKey
                )
                : AllColumns.Where(p =>
                    !(
                        p.Property.Ignored
                        || p.Property.IsReadOnly
                        || p.Property.KeyType == KeyType.Identity
                        || p.Property.KeyType == KeyType.Assigned
                        || p.Property.KeyType == KeyType.SequenceIdentity
                    )
                )
        ).ToList();
        ...
        var setSql = columns
            .Where(c =>
                colsToUpdate == null
                || colsToUpdate?.Any(cu =>
                    cu.PropertyName.Equals(
                        c.Property.ColumnName,
    ...
    private static IMemberMap GetPropertyMap(IClassMapper mainMap, MemberInfo propertyInfo)
    {
        if (!mainMap.Properties.Any(p => p.MemberInfo == propertyInfo))
            throw new KeyNotFoundException(
                $"The property {propertyInfo.Name} was not found in {mainMap.EntityType.Name} entity"
            );
        return mainMap
            .Properties.Where(p => p.MemberInfo == propertyInfo)
            .Select(propertyMap => propertyMap)
            .Single();
    }

    private string GetJointTables(
        IClassMapper mainMap,
        Table table,
        IDictionary<string, object> parameters,
        IList<IReferenceMap> includedProperties = null
    )
    {
        ...
        var useIncludedProperties = includedProperties?.Any(r =>
            r.PropertyInfo == table.PropertyInfo
        );

        if (useIncludedProperties == true)
        {
            var map = includedProperties.FirstOrDefault(r =>
                r.PropertyInfo.PropertyType == table.PropertyInfo.PropertyType
                && r.Identity == table.Identity
            );
        ...
        else
        {
            joinType = mainMap
                .References.Where(r =>
                    r.ParentIdentity == mainMap.Identity
                    && r.PropertyInfo == table.PropertyInfo
                    && r.EntityType == table.EntityType
                )
                .Select(r => r.JoinType)
                .SingleOrDefault();
        }
        ...
        sql.Append(' ')
            .Append(Enum.GetName(typeof(JoinType), joinType))
            .Append(" join ")
            .Append(table.Name)
            .Append(' ')
            .Append(table.Alias)
        ...
        var properties = mainMap
            .References.Where(r =>
                r.ParentIdentity == mainMap.Identity && r.PropertyInfo == table.PropertyInfo
            )
        ...
        if (properties.Count > 0)
        {
            ...
            {
                var leftPropertyMap = GetPropertyMap(
                    leftClasMap,
                    property.LeftProperty.PropertyInfo
                );
                var rightPropertyMap = GetPropertyMap(
                    rightClasMap,
                    property.RightProperty.PropertyInfo
                );
                ...
                sql.Append(' ')
                    .Append(table.Alias)
                    .Append('.')
                    .Append(leftPropertyMap.ColumnName)
                    .Append(' ')
                    .Append(property.ComparatorSignal)
                    .Append(' ')
                    .Append(parent.Alias)
                    .Append('.')
                    .Append(rightPropertyMap.ColumnName);
                joins.AppendLine(sql.ToString());
    ...
    public virtual string GetAllJointTables(
        IClassMapper mainMap,
        string mainTable,
        IDictionary<string, object> parameters,
    ...
    private string GetJoinFromSqlInjection(SqlInjection sqlInjection)
    {
        ...
        var columName = map
            .Properties.Where(p =>
                p.Name.Equals(sqlInjection.Property, StringComparison.InvariantCultureIgnoreCase)
            )
    ...
    )
    {
        ...
        var joints = _includeRelacionalEntities
            ? GetAllJointTables(
                map,
                map.TableName,
                parameters,
            ...
            : "";
        var sqlInjection = GetJoinFromSqlInjection(
            Configuration.GetOrSetSqlInjection(map.EntityType)
        );
    ...
    {
        return Configuration.Dialect.GetTableName(
            map.SchemaName,
            map.TableName,
            useAlias ? GetAliasFromTableName(map.Identity) : null
        );
    }
    ...
    )
    {
        ...
        if (AllColumns?.Any(c => c.Property == property) == true)
        {
            ...
            {
                c.ClassMapper.GetType()
                    .GetProperty("SimpleAlias")
                    .SetValue(c.ClassMapper, GetTableName(c.ClassMapper, c), null);
            }
        }

        var alias = (property.ColumnName != property.Name && includeAlias) ? property.Name : null;
        var prefix = includePrefix
            ? (!string.IsNullOrEmpty(map.SimpleAlias) ? map.SimpleAlias : GetTableName(map))
            : null;
    ...
    {
        var propertyMap = map?.Properties?.SingleOrDefault(p =>
            propertyName.Equals(p.Name, StringComparison.InvariantCultureIgnoreCase)
        );
        if (propertyMap == null)
        {
            throw new ArgumentException(
                string.Format("Could not find '{0}' in Mapping.", propertyName)
            );
        }
    ...
    {
        return Tables.Single(t =>
            t.ParentEntityType == parentEntityType && t.EntityType == entityType
        );
    }
    ...
    {
        return Tables.Single(t =>
            t.Identity.Equals(identity) && t.ParentIdentity.Equals(parentIdentity)
        );
    }

    private IClassMapper GetVirtualClassMapper(IClassMapper mapper)
    {
        var assemblyBuilder = DapperExtensionsReflectionHelper.CreateAssemblyBuilder(
            mapper.EntityType.Assembly.GetName().Name
        );
        var moduleBuilder = DapperExtensionsReflectionHelper.CreateModuleBuilder(
            assemblyBuilder,
            "VirtualModules.dll"
        );

        var tbEntity = DapperExtensionsReflectionHelper.CreateTypeBuilder(
            moduleBuilder,
            mapper.EntityType.Name,
            mapper.EntityType
        );
        var virtualEntity = DapperExtensionsReflectionHelper.CreateVirtualType(
            tbEntity,
            mapper.EntityType
        );

        var tbMapper = DapperExtensionsReflectionHelper.CreateTypeBuilder(
            moduleBuilder,
            mapper.EntityType.Name + "Map",
            mapper.GetType()
        );
    ...
    private IClassMapper CreateVirtualClassMap(Type entityType, Guid parentIdentity)
    {
        ...
        virtualClassMapper
            .GetType()
            .GetMethod("SetParentIdentity")
            .Invoke(virtualClassMapper, argsP);

        return virtualClassMapper;
    }

    private IReferenceMap GetIdentityFromIncludedProperties(
        IList<IReferenceMap> includedProperties,
        PropertyInfo property,
        Guid parentIdentity
    )
    {
        return includedProperties.FirstOrDefault(p =>
            p.ParentIdentity == parentIdentity
            && p.PropertyInfo == property
    ...
    private IClassMapper GetVirtualReferenceMap(
        ref Table table,
        bool isVirtual,
        Guid parentIdentity,
        Type parentType,
    ...
    )
    {
        if (isVirtual)
        {
            var identity = TablesAdded
                .Where(c => c.Identity == parentIdentity && c.IsVirtual)
                .ToList();
            if (identity.Count > 0)
            ...
            var newIdentity = GetIdentityFromIncludedProperties(
                includedProperties,
                table.PropertyInfo,
                table.ParentIdentity
            );
    ...
    private void ProcessRelationationalIdentities(
        ref IClassMapper mapper,
        ref IClassMapper parent,
    ...
    )
    {
        if (includedProperties?.Count > 0)
            ...
            else
            {
                ...
                var childIdentityFromIncluded = includedProperties.FirstOrDefault(i =>
                    TablesAdded.Any(a => a.Identity == i.ParentIdentity)
                    && i.ParentIdentity == parentIdentity
                    && i.PropertyInfo == propertyInfo
                );

                if (childIdentityFromIncluded != null)
                {
                    mapper.SetIdentity(childIdentityFromIncluded.Identity);
                    SetReferencePropertiesParentIdentity(
                        mapper,
                        childIdentityFromIncluded.Identity
                    );
                }
            }
    }

    private IList<Table> ProcessReference(
        IReferenceMap reference,
        IClassMapper mapper,
        IClassMapper parent,
        IClassMapper virtualReferenceMap,
        IList<IReferenceMap> includedProperties
    )
    {
        IClassMapper getTopParentMap(IClassMapper virtualMap, IReferenceMap reference) =>
            (
                virtualMap != null
                && includedProperties.Any(i =>
                    i.ParentIdentity == virtualMap.Identity
                    && i.PropertyInfo == reference.PropertyInfo
                )
            )
                ? virtualMap
                : mapper;
        ...
        var isVirtual = TablesAdded.Any(a =>
            a.Identity == map.Identity && a.ParentIdentity == parent.Identity
        );
        if (
            !isVirtual
            || (isVirtual && !TablesAdded.Any(a => a.ParentIdentity == map.Identity && a.IsVirtual))
        )
        {
    ...
    private IList<Table> ProcessReferences(
        IClassMapper mapper,
        IClassMapper parent,
        IClassMapper virtualReferenceMap,
        IList<IReferenceMap> includedProperties
    )
    {
        IEnumerable<IReferenceMap> getReferences(IClassMapper mapper) =>
            mapper.References.Where(r =>
                ...
                && includedProperties.Any(a =>
                    a.PropertyInfo == r.PropertyInfo && a.ParentIdentity == mapper.Identity
                )
            );
    ...
    private IList<Table> GetAllMappedTables(
        IClassMapper parentClassMapper,
        IClassMapper topParentMap,
        PropertyInfo propertyInfo,
        bool isVirtualMap = false,
        IList<IReferenceMap> includedProperties = null
    )
    {
        var tables = new List<Table>();
        ...
        ProcessRelationationalIdentities(
            ref parentClassMapper,
            ref topParentMap,
            propertyInfo,
            includedProperties
        );
        ...
        IClassMapper virtualReferenceMap = GetVirtualReferenceMap(
            ref _table,
            isVirtualMap,
            topParentMap.Identity,
            parentClassMapper.EntityType,
            includedProperties
        );

        if (
            parentClassMapper.Identity == topParentMap.Identity
            || includedProperties.Any(a => a.Identity == _table.Identity)
        )
        {
            tables.Add(_table);
    ...
    public virtual string BuildSelectColumns(
        IClassMapper classMap,
        IList<IProjection> colsToSelect,
        IList<IReferenceMap> includedProperties = null
    )
    {
        ...
        var columns = AllColumns
            .Where(col =>
                !col.Property.Ignored
                && (
                    colsToSelect == null
                    || colsToSelect?.Any(c =>
                        c.PropertyName.Equals(
                            col.Property.ColumnName,
                            StringComparison.OrdinalIgnoreCase
                        )
                    ) == true
                )
        ...
        return string.IsNullOrEmpty(result)
            ? throw new NotSupportedException("Query with empty ClassMapper is not supported.")
            : result;
    }

    private string GetAliasFromTableName(Guid identity)
    {
        return Tables.Where(x => x.Identity == identity).Select(s => s.Alias).FirstOrDefault();
    }
    ...
    public string GetReference(Table table, string parentReference = "")
    {
        ...
        {
            _reference =
                (!string.IsNullOrEmpty(parentReference))
                    ? table.PropertyInfo.Name + "_" + parentReference
                    : table.PropertyInfo.Name;
    ...
    private IEnumerable<IColumn> GetColumns(Table table)
    {
        ...
        string getParentReference(IMemberMap map) =>
            map.ParentProperty != null ? map.ParentProperty.Name : string.Empty;

        var map = table.ClassMapper ?? Configuration.GetMap(table.EntityType);
        var columnIndex = 0;

        return map?.Properties?.Where(p =>
                    (
                        map?.References == null
                        || map?.References?.Any(r => r.PropertyInfo?.Name == p.Name) == false
                    )
                    && map?.Properties?.Any(mp => mp.ParentProperty == p) == false
                )
                .Select(m =>
                {
                    var alias = !string.IsNullOrEmpty(reference)
                        ? reference + getParentReference(m) + "_" + m.Name
                        : getParentReference(m) + m.Name;
                    return new Column(
                        string.IsNullOrEmpty(alias) ? $"Column{++columnIndex}" : alias,
                        m,
                        m.ClassMapper,
                        table
                    );
    ...
    {
        return map
            .References.SelectMany(c => c.ReferenceProperties)
            .Select(r => r)
            .Distinct()
            .ToList();
    }
