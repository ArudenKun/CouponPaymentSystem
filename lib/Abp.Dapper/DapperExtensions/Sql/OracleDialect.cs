...
public class OracleDialect : SqlDialectBase
{
    ...
    )
    {
        if (string.IsNullOrEmpty(partitionBy))
            throw new ArgumentNullException(
                nameof(partitionBy),
                $"{nameof(partitionBy)} cannot be null."
            );
        ...
        return string.Format(
            GetSetSql(sql, toSkip, topLimit, parameters),
            partitionBy,
            partitionBy,
            partitionBy,
            sql,
            setCompare(partitionBy, "liner", "ss_dapper_1")
        );
    }
    ...
    )
    {
        ...
        sb.AppendLine(
            "inner join (select {0}, ROW_NUMBER() OVER (ORDER BY {1} ASC) LINE_NUMBER from ("
        );
    ...
    public override string GetTableName(string schemaName, string tableName, string alias)
    {
        if (string.IsNullOrWhiteSpace(tableName))
        {
            throw new ArgumentNullException(
                nameof(tableName),
                $"{nameof(tableName)} cannot be null or empty."
            );
        }
    ...
    public override string GetColumnName(string prefix, string columnName, string alias)
    {
        if (string.IsNullOrEmpty(columnName))
            throw new ArgumentNullException(
                nameof(columnName),
                $"{nameof(columnName)} cannot be null or empty."
            );
