...
public class DB2Dialect : SqlDialectBase
{
    ...
    public override string GetPagingSql(
        string sql,
        int page,
        int resultsPerPage,
    ...
    )
    {
        return GetSetSql(
            sql,
            GetStartValue(page, resultsPerPage) + 1,
            (page * resultsPerPage),
            parameters
        );
    }

    public override string GetSetSql(
        string sql,
        int firstResult,
        int maxResults,
        IDictionary<string, object> parameters
    )
    {
        ...
        var projectedColumns = GetColumnNames(sql)
            .Aggregate(
                new StringBuilder(),
                (sb, s) =>
                    (sb.Length == 0 ? sb : sb.Append(", ")).Append(GetColumnName("_TEMP", s, null)),
                sb => sb.ToString()
            );
        ...
        var result = string.Format(
            "SELECT {0} FROM ({1}) AS \"_TEMP\" WHERE {2} BETWEEN @_pageStartRow AND @_pageEndRow",
            projectedColumns.Trim(),
            newSql,
            GetColumnName("_TEMP", "_ROW_NUMBER", null)
        );
    ...
    {
        var orderByIndex = sql.LastIndexOf(
            " ORDER BY ",
    ...
    protected virtual int GetSelectEnd(string sql)
    {
        return sql.IndexOf("SELECT")
            + (
                sql.TrimStart()
                    .StartsWith("SELECT DISTINCT", StringComparison.InvariantCultureIgnoreCase)
                    ? 15
                    : 6
            );
    }
    ...
    public override string GetDatabaseFunctionString(
