...
public class MustHaveTenantDapperQueryFilter : IDapperQueryFilter
{
    ...
    private int TenantId
    {
        get
        {
            DataFilterConfiguration filter =
                _currentUnitOfWorkProvider.Current.Filters.FirstOrDefault(x =>
                    x.FilterName == FilterName
                );
    ...
    public IFieldPredicate ExecuteFilter<TEntity, TPrimaryKey>()
        where TEntity : class, IEntity<TPrimaryKey>
    {
        IFieldPredicate predicate = null;
        if (typeof(IMustHaveTenant).IsAssignableFrom(typeof(TEntity)) && IsEnabled)
        {
            predicate = Predicates.Field<TEntity>(
                f => (f as IMustHaveTenant).TenantId,
                Operator.Eq,
                TenantId
            );
        }
        ...
        where TEntity : class, IEntity<TPrimaryKey>
    {
        if (typeof(IMustHaveTenant).IsAssignableFrom(typeof(TEntity)) && IsEnabled)
        {
            ...
            if (predicate == null)
            {
                predicate = ExpressionUtils.MakePredicate<TEntity>(
            ...
            else
            {
                ...
                MemberExpression memberExpr = Expression.Property(
                    paramExpr,
                    nameof(IMustHaveTenant.TenantId)
                );
                BinaryExpression body = Expression.AndAlso(
                    ...
                    Expression.Equal(
                        memberExpr,
                        Expression.Constant(TenantId, propType.PropertyType)
                    )
                );
