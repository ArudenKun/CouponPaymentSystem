        ...
        ISingletonDependency
{
        ...
        where TArgs : notnull
    {
        ...
        var jobInfoId = jobInfo.Id.ToString();
        ...
        try
        {
            ...
            try
            {
                var jobExecuteMethod =
                    job.Object.GetType().GetTypeInfo().GetMethod(nameof(IBackgroundJob<>.Execute))
                    ?? job.Object.GetType()
                        .GetTypeInfo()
                        .GetMethod(nameof(IAsyncBackgroundJob<>.ExecuteAsync));
                ...
                var argsObj = JsonSerializer.Deserialize(jobInfo.JobArgs, argsType);

                if (jobExecuteMethod.Name == nameof(IAsyncBackgroundJob<>.ExecuteAsync))
                {
                    await ((Task)jobExecuteMethod.Invoke(job.Object, [argsObj]));
                }
                else
                {
                    jobExecuteMethod.Invoke(job.Object, [argsObj]);
                }
