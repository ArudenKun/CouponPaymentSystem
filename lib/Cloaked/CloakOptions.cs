...
/// <summary>
/// Base configuration options for Cloak.
/// </summary>
        ...
        set
        {
            if (value < 0)
                throw new ArgumentOutOfRangeException(
                    nameof(value),
                    "MinLength must be 0 or greater."
                );
            field = value;
        }
    } = 6;
    ...
    private static void ValidateAlphabet(string alphabet)
    {
        if (string.IsNullOrWhiteSpace(alphabet))
            throw new ArgumentException(
                "Alphabet cannot be null, empty, or whitespace.",
                nameof(alphabet)
            );

        if (alphabet.Length < 3)
            throw new ArgumentException(
                "Alphabet must contain at least 3 characters.",
                nameof(alphabet)
            );
        ...
        foreach (char c in alphabet)
        {
            if (!uniqueChars.Add(c))
                throw new ArgumentException(
                    $"Alphabet contains duplicate character '{c}'. All characters must be unique.",
                    nameof(alphabet)
                );
        }
        ...
        foreach (char c in alphabet)
        {
            if (char.IsWhiteSpace(c))
                throw new ArgumentException(
                    $"Alphabet contains whitespace character at position {alphabet.IndexOf(c)}. Whitespace characters are not allowed.",
                    nameof(alphabet)
                );
        }
        ...
        foreach (char c in alphabet)
        {
            if (!IsUrlSafeCharacter(c))
                throw new ArgumentException(
                    $"Alphabet contains unsafe character '{c}' at position {alphabet.IndexOf(c)}. Only alphanumeric characters (A-Z, a-z, 0-9) are allowed to ensure compatibility with web frameworks and routing.",
                    nameof(alphabet)
                );
        }
    }
