...
/// </summary>
public sealed partial class SqlDistributedSemaphore
    : IInternalDistributedSemaphore<SqlDistributedSemaphoreHandle>
{
    ...
    /// </summary>
    public SqlDistributedSemaphore(
        string name,
        int maxCount,
    ...
    private SqlDistributedSemaphore(
        string name,
        int maxCount,
        Func<string, IDbDistributedLock> createInternalLockFromName
    )
    {
        if (maxCount < 1)
        {
    ...
    async ValueTask<SqlDistributedSemaphoreHandle?> IInternalDistributedSemaphore<SqlDistributedSemaphoreHandle>.InternalTryAcquireAsync(
        TimeoutValue timeout,
        CancellationToken cancellationToken
    )
    {
        var handle = await this
