...
internal sealed class SqlSemaphore(int maxCount) : IDbSynchronizationStrategy<SqlSemaphore.Cookie>
{
    ...
    #region ---- Execution ----
    public async ValueTask<Cookie?> TryAcquireAsync(
    ...
    )
    {
        ...
        if (!cancellationToken.CanBeCanceled || timeout.IsZero)
        {
            ...
            return await ProcessAcquireResultAsync(
                    command.Parameters,
                    timeout,
                    cancellationToken,
                    out markerTableName,
                    out var ticketLockName
                )
        ...
        using (var command = CreateTextCommand(connection, operationTimeout: timeout))
        {
            ...
            await command.ExecuteNonQueryAsync(CancellationToken.None).ConfigureAwait(false);
            if (
                await ProcessAcquireResultAsync(
                        command.Parameters,
                        timeout,
                        cancellationToken,
                        out markerTableName,
        ...
        using (var command = CreateTextCommand(connection, operationTimeout: timeout))
        {
            ...
            try
            {
                ...
                await command
                    .ExecuteNonQueryAsync(cancellationToken, disallowAsyncCancellation: true)
                    .ConfigureAwait(false);
            }
            catch when (cancellationToken.IsCancellationRequested)
            {
                ...
                using (
                    var cleanupCommand = CreateTextCommand(
                        connection,
    ...
    {
        using var command = CreateTextCommand(
            connection,
        ...
        this.AddCommonParameters(
            command,
            resourceName,
            markerTableName: lockCookie.MarkerTable,
            ticketLockName: lockCookie.Ticket
        );
        await command.ExecuteNonQueryAsync(CancellationToken.None).ConfigureAwait(false);
    }
    ...
    public sealed class Cookie(string ticket, string markerTable)
    {
        ...
        public string MarkerTable { get; } =
            markerTable ?? throw new ArgumentNullException(nameof(markerTable));
    }
    #endregion
    ...
    )
    {
        ...
        {
            case SuccessCode:
                ticketLockName = (string)
                    ((IDbDataParameter)parameters[TicketLockNameParameter]).Value;
                markerTableName = (string)
                    ((IDbDataParameter)parameters[MarkerTableNameParameter]).Value;
            ...
            case FailedToAcquireWithSpaceRemainingCode:
                throw new InvalidOperationException(
                    $"An internal semaphore algorithm error ({resultCode}) occurred: failed to acquire a ticket despite indication that tickets are available"
                );
            ...
            case AllTicketsHeldByCurrentSessionCode:
                ...
                if (timeout.IsInfinite)
                {
                    throw new DeadlockException(
                        "Deadlock detected: attempt to acquire the semaphore cannot succeed because all tickets are held by the current connection"
                    );
                }
            ...
            default:
                ...
                async ValueTask<bool> FailAsync()
                {
                    if (resultCode < 0)
                    {
                        await SqlApplicationLock
                            .ParseExitCodeAsync(resultCode, timeout, cancellationToken)
                            .ConfigureAwait(false);
                    }
                    throw new InvalidOperationException(
                        $"Unexpected semaphore algorithm result code {resultCode}"
                    );
                }
        }
    }

    private static DatabaseCommand CreateTextCommand(
        DatabaseConnection connection,
    ...
    private void AddCommonParameters(
        ...
        TimeoutValue? timeout = null,
        string? markerTableName = null,
        string? ticketLockName = null
    )
    {
        ...
        var ticket = command.AddParameter(
            TicketLockNameParameter,
            ticketLockName,
            type: DbType.String
        );
        ...
        var markerTable = command.AddParameter(
            MarkerTableNameParameter,
            markerTableName,
            type: DbType.String
        );
    ...
    public static string ToSafeName(string semaphoreName)
    {
        ...
        var maxBaseNameLength =
            MaxTableNameLength - (nameHash.Length + Suffix.Length + AdditionalSuffixMaxLength);
        var baseName =
            nameWithoutInvalidCharacters.Length <= maxBaseNameLength
        ...
        for (var i = 0; i < semaphoreName.Length; ++i)
        {
            ...
            if (!IsAsciiLetterOrDigit(@char))
            {
                if (modifiedName == null)
                {
                    ...
                    for (var j = 0; j < i; ++j)
                    {
                        modifiedName.Append(semaphoreName[j]);
                    }
                }
    ...
    private static readonly Lazy<string> AcquireNonCancelableQuery =
            new(() =>
                Merge(
                    CreateCommonVariableDeclarationsSql(
                        includePreambleLock: true,
                        includeBusyWaitLock: true,
                        includeTryAcquireOnceVariables: true
                    ),
                    CreateAcquirePreambleSql(willRetryInSeparateQueryAfterPreamble: null),
                    CreateAcquireSql(cancelable: false),
                    CreateCodaSql(
                        includePreambleLockRelease: true,
                        includeBusyWaitLockRelease: true
                    )
                )
            ),
        ...
        AcquireCancelablePreambleQuery =
            new(() =>
                Merge(
                    CreateCommonVariableDeclarationsSql(
                        includePreambleLock: true,
                        includeBusyWaitLock: false,
                        includeTryAcquireOnceVariables: true
                    ),
                    CreateAcquirePreambleSql(willRetryInSeparateQueryAfterPreamble: true),
                    CreateCodaSql(
                        includePreambleLockRelease: true,
                        includeBusyWaitLockRelease: false
                    )
                )
            ),
        AcquireCancelableQuery =
            new(() =>
                Merge(
                    CreateCommonVariableDeclarationsSql(
                        includePreambleLock: false,
                        includeBusyWaitLock: true,
                        includeTryAcquireOnceVariables: true
                    ),
                    CreateAcquireSql(cancelable: true),
                    CreateCodaSql(
                        includePreambleLockRelease: false,
                        includeBusyWaitLockRelease: true
                    )
                )
            ),
        CancellationCleanupQuery =
            new(() =>
                Merge(
                    CreateCommonVariableDeclarationsSql(
                        includePreambleLock: false,
                        includeBusyWaitLock: true,
                        includeTryAcquireOnceVariables: false
                    ),
    ...
    private static string CreateCodaSql(
        bool includePreambleLockRelease,
        bool includeBusyWaitLockRelease
    )
    {
        return $@"
                CODA:
