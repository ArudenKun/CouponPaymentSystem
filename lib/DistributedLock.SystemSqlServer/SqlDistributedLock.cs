...
public sealed partial class SqlDistributedLock : IInternalDistributedLock<SqlDistributedLockHandle>
{
    ...
    ValueTask<SqlDistributedLockHandle?> IInternalDistributedLock<SqlDistributedLockHandle>.InternalTryAcquireAsync(
        ...
        this
            ._internalLock.TryAcquireAsync(
                timeout,
                SqlApplicationLock.ExclusiveLock,
                cancellationToken,
                contextHandle: null
            )
            .Wrap(h => new SqlDistributedLockHandle(h));

    internal static IDbDistributedLock CreateInternalLock(
        string name,
        string connectionString,
        Action<SqlConnectionOptionsBuilder>? optionsBuilder
    )
    {
        ...
        var (keepaliveCadence, useTransaction, useMultiplexing) =
            SqlConnectionOptionsBuilder.GetOptions(optionsBuilder);

        return useMultiplexing
            ? new OptimisticConnectionMultiplexingDbDistributedLock(
                name,
                connectionString,
                SqlMultiplexedConnectionLockPool.Instance,
                keepaliveCadence
            )
            : new DedicatedConnectionOrTransactionDbDistributedLock(
                name,
                () => new SqlDatabaseConnection(connectionString),
    ...
    internal static IDbDistributedLock CreateInternalLock(string name, IDbConnection connection)
    {
        return connection == null
            ...
            : new DedicatedConnectionOrTransactionDbDistributedLock(
                name,
                () => new SqlDatabaseConnection(connection)
            );
    }
    ...
    internal static IDbDistributedLock CreateInternalLock(string name, IDbTransaction transaction)
    {
        return transaction == null
            ...
            : new DedicatedConnectionOrTransactionDbDistributedLock(
                name,
                () => new SqlDatabaseConnection(transaction)
            );
    }
}
