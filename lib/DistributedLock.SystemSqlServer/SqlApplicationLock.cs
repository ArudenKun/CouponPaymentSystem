...
internal sealed class SqlApplicationLock : IDbSynchronizationStrategy<object>
{
    ...
    )
    {
        try
        {
            return await this.ExecuteAcquireCommandAsync(
                    connection,
                    resourceName,
                    timeout,
                    cancellationToken
    ...
    ValueTask IDbSynchronizationStrategy<object>.ReleaseAsync(
        DatabaseConnection connection,
        string resourceName,
        object lockCookie
    ) => ExecuteReleaseCommandAsync(connection, resourceName, isTry: false);

    private async Task<bool> ExecuteAcquireCommandAsync(
        DatabaseConnection connection,
        string lockName,
        TimeoutValue timeout,
        CancellationToken cancellationToken
    )
    {
        ...
        return await ParseExitCodeAsync((int)returnValue.Value, timeout, cancellationToken)
            .ConfigureAwait(false);
    }

    private static async ValueTask ExecuteReleaseCommandAsync(
        DatabaseConnection connection,
        string lockName,
        bool isTry
    )
    {
        ...
        await ParseExitCodeAsync((int)returnValue.Value, TimeSpan.Zero, CancellationToken.None)
            .ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        {
            returnValue = command.AddParameter(
                "Result",
                type: DbType.Int32,
            ...
            var alternateOwnerHasLockCheck =
    ...
    private static DatabaseCommand CreateReleaseCommand(
        DatabaseConnection connection,
        string lockName,
        bool isTry,
        out IDbDataParameter returnValue
    ...
    public static async ValueTask<bool> ParseExitCodeAsync(
        int exitCode,
        TimeoutValue timeout,
        CancellationToken cancellationToken
    )
        ...
        switch (exitCode)
        {
            ...
            case -999: // parameter / unknown
                throw new ArgumentException(
                    GetErrorMessage(exitCode, "parameter validation or other error")
                );
            ...
            case InvalidUpgradeExitCode:
                ...
                throw new InvalidOperationException(
                    "Cannot upgrade to an exclusive lock because the update lock is not held"
                );
            case AlreadyHeldExitCode:
                return timeout.IsZero ? false
                    : timeout.IsInfinite
                        ? throw new DeadlockException(
                            "Attempted to acquire a lock that is already held on the same connection"
                        )
    ...
    private static string GetErrorMessage(int exitCode, string type) =>
        $"The request for the distributed lock failed with exit code {exitCode} ({type})";
