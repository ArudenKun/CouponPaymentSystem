...
/// </summary>
public sealed partial class SqlDistributedReaderWriterLock
    ...
    >
{
    ...
    /// </summary>
    public SqlDistributedReaderWriterLock(
        string name,
        ...
        : this(
            name,
            exactName,
            n => SqlDistributedLock.CreateInternalLock(n, connectionString, options)
        ) { }
    ...
    private SqlDistributedReaderWriterLock(
        string name,
        bool exactName,
        Func<string, IDbDistributedLock> internalLockFactory
    )
    {
        if (exactName)
        {
            ...
            if (name.Length > MaxNameLength)
            {
                throw new FormatException(
                    $"{nameof(name)}: must be at most {MaxNameLength} characters"
                );
            }
            this.Name = name;
        }
    ...
    async ValueTask<SqlDistributedReaderWriterLockUpgradeableHandle?> IInternalDistributedUpgradeableReaderWriterLock<
        SqlDistributedReaderWriterLockHandle,
        SqlDistributedReaderWriterLockUpgradeableHandle
    ...
    {
        var innerHandle = await this
            ._internalLock.TryAcquireAsync(
                timeout,
                SqlApplicationLock.UpdateLock,
                cancellationToken,
                contextHandle: null
            )
            .ConfigureAwait(false);
        return innerHandle != null
            ? new SqlDistributedReaderWriterLockUpgradeableHandle(innerHandle, this._internalLock)
            : null;
    }
    ...
    )
    {
        var innerHandle = await this
            ._internalLock.TryAcquireAsync(
                timeout,
                isWrite ? SqlApplicationLock.ExclusiveLock : SqlApplicationLock.SharedLock,
                cancellationToken,
                contextHandle: null
            )
            .ConfigureAwait(false);
        return innerHandle != null
            ? new SqlDistributedReaderWriterLockNonUpgradeableHandle(innerHandle)
            : null;
    }
}
