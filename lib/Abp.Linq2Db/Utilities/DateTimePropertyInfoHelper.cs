...
namespace Abp.Linq2Db.Utilities;
...
internal static class DateTimePropertyInfoHelper
{
    ...
    public static void NormalizeDatePropertyKinds(object entity, Type entityType)
    {
        ...
        dateTimePropertyInfos.ComplexTypePropertyPaths.ForEach(propertyPath =>
        {
            var property = (PropertyInfo)
                ReflectionHelper.GetPropertyByPath(entity, entityType, propertyPath);
            ...
            var dateTime = (DateTime?)
                ReflectionHelper.GetValueByPath(entity, entityType, propertyPath);
            if (dateTime.HasValue)
            {
                ReflectionHelper.SetValueByPath(
                    entity,
                    entityType,
                    propertyPath,
    ...
    private static EntityDateTimePropertiesInfo FindDatePropertyInfosForType(Type entityType)
    {
        ...
        var complexTypeProperties = entityType
            .GetProperties()
            .Where(p =>
        ...
        {
            AddComplexTypeDateTimePropertyPaths(
                entityType.FullName + "." + complexTypeProperty.Name,
                complexTypeProperty,
    ...
    private static void AddComplexTypeDateTimePropertyPaths(
        string pathPrefix,
    ...
    )
    {
        ...
        var complexTypeDateProperties = complexProperty
            ...
            .Select(p => pathPrefix + "." + p.Name)
            .ToList();

        complexTypeDateTimePropertyPaths.AddRange(complexTypeDateProperties);

        var complexTypeProperties = complexProperty
            .PropertyType.GetProperties()
            .Where(p =>
                p.PropertyType.IsDefined(typeof(OwnedAttribute), true)
                && !p.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)
        ...
        {
            AddComplexTypeDateTimePropertyPaths(
                pathPrefix + "." + complexTypeProperty.Name,
                complexTypeProperty,
                complexTypeDateTimePropertyPaths
            );
        }
