...
internal class ApiControllerActionBuilder<T> : IApiControllerActionBuilder<T>
{
    ...
    public ApiControllerActionBuilder(
        ApiControllerBuilder<T> apiControllerBuilder,
    ...
    private HttpVerb GetNormalizedVerb(bool conventionalVerbs)
    {
        ...
        {
            var conventionalVerb = DynamicApiVerbHelper.GetConventionalVerbForMethodName(
                ActionName
            );
            if (
                conventionalVerb == HttpVerb.Get
                && !HasOnlyPrimitiveIncludingNullableTypeParameters(Method)
            )
            {
