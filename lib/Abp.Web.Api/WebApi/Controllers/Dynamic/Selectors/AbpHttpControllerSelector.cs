...
namespace Abp.WebApi.Controllers.Dynamic.Selectors
{
    ...
    public class AbpHttpControllerSelector : DefaultHttpControllerSelector
    {
        ...
        public AbpHttpControllerSelector(
        ...
        public override HttpControllerDescriptor SelectController(HttpRequestMessage? request)
        {
            ...
            if (!routeData.Values.TryGetValue("serviceNameWithAction", out var value))
            {
                return base.SelectController(request);
            }

            var serviceNameWithAction = (string)value;

            //Normalize serviceNameWithAction
            if (serviceNameWithAction.EndsWith("/"))
            {
                serviceNameWithAction = serviceNameWithAction.Substring(
                    0,
                    serviceNameWithAction.Length - 1
                );
            ...
            {
                if (
                    !DynamicApiServiceNameHelper.IsValidServiceNameWithAction(serviceNameWithAction)
                )
                {
                    return base.SelectController(request);
                }

                var serviceName = DynamicApiServiceNameHelper.GetServiceNameInServiceNameWithAction(
            ...
            )
            {
                Properties =
                {
                    ["__AbpDynamicApiControllerInfo"] = controllerInfo,
                    ["__AbpDynamicApiHasActionName"] = hasActionName,
                },
            };
