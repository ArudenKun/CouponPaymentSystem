...
internal class AngularActionScriptWriter
{
    ...
    public AngularActionScriptWriter(
    ...
    public virtual void WriteTo(StringBuilder script)
    {
        script.AppendLine(
            "                this"
                + ProxyScriptingJsFuncHelper.WrapWithBracketsOrWithDotPrefix(
                    _actionInfo.ActionName.ToCamelCase()
                )
                + " = function ("
                + ActionScriptingHelper.GenerateJsMethodParameterList(
                    _actionInfo.Method,
                    "httpParams"
                )
                + ") {"
        ...
        {
            script.AppendLine(
                "                        params: " + ActionScriptingHelper.GenerateBody(_actionInfo)
            );
        }
        else
        {
