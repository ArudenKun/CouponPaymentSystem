...
{ajaxCallParameters}
    ...
    public JQueryActionScriptGenerator(
    ...
    public virtual string GenerateMethod()
    {
        ...
        var jsMethodParameterList = ActionScriptingHelper.GenerateJsMethodParameterList(
            _actionInfo.Method,
            "ajaxParams"
        );
        ...
        var jsMethod = JsMethodTemplate
            .Replace(
                "{jsMethodName}",
                ProxyScriptingJsFuncHelper.WrapWithBracketsOrWithDotPrefix(jsMethodName)
            )
    ...
    protected string GenerateAjaxCallParameters()
    {
        ...
        script.AppendLine(
            "            type: '" + _actionInfo.Verb.ToString().ToUpperInvariant() + "',"
        );
