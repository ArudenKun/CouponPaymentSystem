...
{
    public static string GenerateUrlWithParameters(
    ...
    )
    {
        ...
        var primitiveParameters = actionInfo
        ...
        var qsBuilderParams = primitiveParameters
            .Select(p =>
                $"{{ name: '{p.Name.ToCamelCase()}', value: " + p.Name.ToCamelCase() + " }"
            )
            .JoinAsString(", ");
    ...
    public static string GenerateJsMethodParameterList(
