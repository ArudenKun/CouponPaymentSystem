...
internal class TypeScriptServiceProxyGenerator : ITransientDependency
{
    ...
    public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
    {
        ...
        script.AppendLine(
            "     export class "
                + _controllerInfo.ServiceName.Substring(
                    _controllerInfo.ServiceName.IndexOf('/') + 1
                )
        );
        script.AppendLine("     {");
        ...
        foreach (var methodInfo in _controllerInfo.Actions.Values)
        {
            ...
            var returnType = TypeScriptHelper.GetTypeContractName(
            ...
            if (returnType == "void")
            {
                script.AppendLine(
                    string.Format(
                        "           public {0} = function ({1}): abp.IPromise ",
                        methodInfo.ActionName.ToCamelCase(),
                        GetMethodInputParameter(methodInfo.Method)
                    )
                );
                script.AppendLine("{");
                ...
                script.AppendLine(
                    "                        url: abp.appPath + '"
                        + ActionScriptingHelper.GenerateUrlWithParameters(
                            _controllerInfo,
                            methodInfo
                        )
                        + "',"
            ...
            else
            {
                script.AppendLine(
                    string.Format(
                        "           public {0} = function ({1}): abp.IGenericPromise<{2}> ",
                        methodInfo.ActionName.ToCamelCase(),
                        GetMethodInputParameter(methodInfo.Method),
                        returnType
                    )
        ...
        script.AppendLine(
            "angular.module('abp').service('abp.services."
    ...
    protected string GenerateTypeScript(Type type)
    {
        ...
        List<Type> newTypes = new List<Type>();
        myscript.AppendLine(
            "     export class " + TypeScriptHelper.GetTypeContractName(type, newTypes)
        );
        myscript.AppendLine("         {");
    ...
    private bool CanAddToBeDone(Type type)
    {
        if (type == typeof(Task))
            return false;
        if (
            _typesToBeDone.Count(z => z.FullName == type.FullName) == 0
            && !TypeScriptHelper.IsIgnorantType(type)
            && !TypeScriptHelper.IsBasicType(type)
            && _doneTypes.Count(z => z.FullName == type.FullName) == 0
        )
            return true;
        return false;
    }
