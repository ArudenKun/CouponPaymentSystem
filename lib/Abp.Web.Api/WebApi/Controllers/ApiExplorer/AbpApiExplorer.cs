...
namespace Abp.WebApi.Controllers.ApiExplorer
{
            ...
            ISingletonDependency
    {
            ...
            foreach (var dynamicApiControllerInfo in dynamicApiControllerInfos)
            {
                ...
                foreach (var dynamicApiActionInfo in dynamicApiControllerInfo.Actions.Values)
                {
                    ...
                    var controllerDescriptor = new DynamicHttpControllerDescriptor(
                        _abpWebApiConfiguration.HttpConfiguration,
                        dynamicApiControllerInfo
                    );
                    controllerDescriptor.ControllerName =
                        controllerDescriptor.ControllerName.Replace("/", "_");
                    var actionDescriptor = new DynamicHttpActionDescriptor(
                        _abpWebApiConfiguration,
                        controllerDescriptor,
                        dynamicApiActionInfo
                    );

                    apiDescription.ActionDescriptor = actionDescriptor;
                    apiDescription.HttpMethod = actionDescriptor.SupportedHttpMethods[0];

                    var actionValueBinder =
                        _abpWebApiConfiguration.HttpConfiguration.Services.GetActionValueBinder();
                    var actionBinding = actionValueBinder.GetBinding(actionDescriptor);

                    apiDescription.ParameterDescriptions.Clear();
                    foreach (
                        var apiParameterDescription in CreateParameterDescription(
                            actionBinding,
                            actionDescriptor
                        )
                    )
                    ...
                    apiDescription.RelativePath =
                        "api/services/"
                        + dynamicApiControllerInfo.ServiceName
                        + "/"
                        + dynamicApiActionInfo.ActionName;
        ...
        private static bool IsApiExplorerDisabled(DynamicApiControllerInfo dynamicApiControllerInfo)
        {
            ...
            {
                if (
                    !RemoteServiceAttribute.IsMetadataExplicitlyEnabledFor(
                        dynamicApiControllerInfo.ServiceInterfaceType
                    )
                )
                {
                    return true;
            ...
            {
                if (
                    RemoteServiceAttribute.IsMetadataExplicitlyDisabledFor(
                        dynamicApiControllerInfo.ServiceInterfaceType
                    )
                )
                {
        ...
        private void SetResponseDescription(
            ApiDescription apiDescription,
            DynamicHttpActionDescriptor actionDescriptor
        )
        {
            var responseDescription = CreateResponseDescription(actionDescriptor);
            var prop2 = typeof(ApiDescription)
                .GetProperties()
                .Single(p => p.Name == "ResponseDescription");
            prop2.SetValue(apiDescription, responseDescription);
        }

        private IList<ApiParameterDescription> CreateParameterDescription(
