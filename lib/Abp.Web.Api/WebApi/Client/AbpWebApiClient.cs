...
public class AbpWebApiClient : ITransientDependency, IAbpWebApiClient
{
    ...
    public virtual async Task<TResult> PostAsync<TResult>(
        string url,
        object input,
        int? timeout = null
    )
        where TResult : class
    {
        ...
        using (var handler = new HttpClientHandler { CookieContainer = cookieContainer })
        {
            ...
            {
                client.Timeout = timeout.HasValue
                ...
                )
                {
                    ...
                    using (var response = await client.PostAsync(url, requestContent))
                    {
                        ...
                        var ajaxResponse = JsonString2Object<AjaxResponse<TResult>>(
    ...
    {
        return JsonConvert.DeserializeObject<TObj>(
            str,
            new JsonSerializerSettings
            {
                ContractResolver = new CamelCasePropertyNamesContractResolver(),
