...
public class AbpApiAuthorizeFilter : IAuthorizationFilter, ITransientDependency
{
    ...
    protected virtual HttpResponseMessage CreateUnAuthorizedResponse(
        HttpActionContext actionContext
    )
    {
        ...
        HttpResponseMessage HandleError(bool wrap)
        {
            ...
            return new HttpResponseMessage(statusCode)
            {
                Content = new ObjectContent<AjaxResponse>(
                    new AjaxResponse(GetUnAuthorizedErrorMessage(statusCode), true),
        ...
        if (
            _abpWebCommonModuleConfiguration.WrapResultFilters.HasFilterForWrapOnError(
                displayUrl,
                out var wrapOnError
            )
        )
